...
import { parseAnalysisContent, type AnalysisView } from "@/lib/analysis";
...
// bajarib, natijani `analyses` jadvaliga saqlaydi. Asosiy mantiq
// (kontekst yig'ish + Gemini chaqiruvi + saqlash) `analysis-engine.ts`da —
// bu funksiya faqat sessiyadan userId olib, xatoni ActionResult'ga o'raydi.
export async function runManualAnalysis(): Promise<ActionResult<AnalysisView>> {
  const userId = await requireUserId();

  try {
    const data = await runAnalysisForUser(userId, "manual");
    revalidatePath("/ai");
    return { ok: true, data };
  } catch (e) {
