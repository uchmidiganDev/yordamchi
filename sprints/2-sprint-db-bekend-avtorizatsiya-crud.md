...
- [x] Drizzle sxema + migratsiyalar (`users`, `goals`, `tasks`, `task_occurrences`, `analyses`) — `users`, `login_tokens`, `goals`, `tasks` jadvallari (hamda `cards`, `expenses`) migratsiya (`drizzle/0001_nosy_human_torch.sql`) bilan Neon'ga qo'llandi; `task_occurrences` va `analyses` jadvallari hamda `tasks.recurrence` ustuni (enum: none/daily/weekly/monthly) migratsiya `drizzle/0003_dazzling_komodo.sql` orqali qo'shildi va Neon'ga qo'llandi.
  > **Eslatma (2026-07-23):** `cards`/`expenses` jadvallari ustidagi CRUD kodi foydalanuvchi so'rovi bilan butunlay olib tashlandi ("Xarajat" bo'limi bekor qilindi — pastga qarang, DoD bo'limi). DB jadvallarining o'zi (schema.ts) ATAYLAB saqlab qolindi, faqat kod o'chirildi.
...
- [x] Barcha ekranlar mock emas, haqiqiy ma'lumot bilan ishlaydi. Maqsad, Vazifa, Xarajat, Tahlil, Sozlamalar sahifalari real DB bilan ishlaydi. AI sahifasi ataylab Sprint 3'ga qoldirilgan (kelishilgan).
  > **Eslatma (2026-07-23):** ~~Xarajat~~ sahifasi (`/xarajat`) foydalanuvchi so'rovi bilan butunlay olib tashlandi — bu band o'sha vaqtda haqiqatan bajarilgan edi, keyinroq foydalanuvchi fikri bilan bekor qilindi. Checkbox tarixiy holatni aks ettirgani uchun o'zgartirilmadi.
...
- **2026-07-16** — Telegram bot istalgan PDF'ni xarajat sifatida saqlash imkoniyati qo'shildi (commit 27647f9): `src/lib/generic-receipt.ts` yaratildi — `parseGenericReceipt(text, fallbackTitle)` funksiyasi istalgan PDF matnidan summani ajratadi (avval valyuta kalit so'zli — "so'm/sum/uzs" — summalarni, bo'lmasa mingliklar ajratgichli raqamlarni qidiradi; bir nechta topilsa eng kattasini jami sifatida oladi), sanani (o'zbekcha oy nomi / dd.mm.yyyy / yyyy-mm-dd formatlarida) va sarlavhani (fayl nomi yoki matnning birinchi qatori) topadi; takroriy saqlashni oldini olish uchun matn sha256 hash'i `ref` sifatida ishlatiladi. `src/lib/expense-from-receipt.ts`ga `saveGenericExpense()` funksiyasi qo'shildi — "PDF cheklar" nomli umumiy kartaga "Chek" kategoriyasi bilan saqlaydi; ref bo'yicha dedup. `src/lib/telegram-bot.ts` PDF handleri qayta ishlandi: avval Paynet formatida sinaydi (karta/tranzaksiya aniqroq), bo'lmasa `parseGenericReceipt` orqali summani topib `saveGenericExpense` bilan saqlaydi; summa topilmasa foydalanuvchiga xabar beradi. Umumiy `getOwnerUser` va `replySaveResult` yordamchi funksiyalari ajratildi. TypeScript va `npm run build` toza o'tdi.
- **2026-07-23** — Foydalanuvchi so'rovi bilan **"Xarajat" bo'limi to'liq olib tashlandi** (yuqoridagi vazifa ro'yxatiga eslatmalar qo'shildi, checkbox'lar o'zgartirilmadi — o'sha ish tarixiy jihatdan haqiqatan bajarilgan edi). Olib tashlandi: `/xarajat` sahifasi va nav tab'i; Telegram botning Paynet chek (matn) va PDF chekni avtomatik xarajat sifatida saqlash funksiyalari (`handleReceiptText()`, `replySaveResult()`, `bot.on("message:document")`); endi hech qayerda ishlatilmagani tasdiqlangach o'chirilgan fayllar — `src/lib/actions/cards.ts`, `src/lib/actions/expenses.ts`, `src/lib/paynet-receipt.ts`, `src/lib/generic-receipt.ts`, `src/lib/expense-from-receipt.ts`, `src/lib/pdf-text.ts`; `next.config.ts`dagi `serverExternalPackages: ["unpdf"]` va `unpdf` npm bog'liqligi. `cards`/`expenses` DB jadvallari va `cardBrand` enum ATAYLAB saqlab qolindi (schema.ts o'zgartirilmadi). "Tahlil" (`/tahlil`) bu kodga bog'liq emasligi tasdiqlangan, boshqa hech narsaga ta'sir qilmadi. Qaror tafsilotlari `CLAUDE.md`da qayd etilgan.
